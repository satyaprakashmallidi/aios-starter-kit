...
Runs all configured collectors (Fathom, Bitrix calendar, Slack), writes to the database,
...
Usage:
    ...
    python scripts/intel/collect_all.py --meetings-only    # Only collect meetings
    python scripts/intel/collect_all.py --slack-only        # Only collect Slack
"""
...
# Ensure scripts/intel/ is on the path
...
def run(meetings_only: bool = False, slack_only: bool = False):
    ...
    if not slack_only:
        ...
        # Bitrix calendar
        try:
            from collect_bitrix_meetings import collect as collect_bitrix
            bitrix_meetings = collect_bitrix()
            if bitrix_meetings:
                meeting_records.extend(bitrix_meetings)
        except Exception as e:
            print(f"Bitrix calendar error: {e}")
